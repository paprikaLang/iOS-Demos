...
class GameScene: SKScene {
    ...
    override init(size:CGSize) {
        ...
        //OpenGL 和 iOS的原生Cocoa coordinates是相反的
        addChild(background)
        
        addChild(gameLayer)
        
        let gameBoardTexture = SKTexture(imageNamed: "gameboard")
        let gameBoard = SKSpriteNode.init(texture: gameBoardTexture, size: CGSize(width: BlockSize * CGFloat(NumColumns), height: BlockSize * CGFloat(NumRows)))
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        let timePassded = lastTick.timeIntervalSinceNow * -1000.0
        if timePassded>tickLengthMillis {
            self.lastTick = Date()
            tick?()
        }
    }
